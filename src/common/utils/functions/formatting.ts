...
export function dropDecimals(number: number): number {
	return parseInt(number.toString());
}

export function roundNearest(number: number, multiple: number): number {
	return Math.round(number / multiple) * multiple;
}

export function camelCase(text: string, lowerCamelCase: boolean = true): string {
	return (text.trim().charAt(0)[lowerCamelCase ? "toLowerCase" : "toUpperCase"]() + text.slice(1)).trim().replaceAll(" ", "");
}

export function convertToNumber(string: string | undefined | null): number {
...
export function textToTime(time: string, partialOptions: Partial<TimeTextOptions> = {}) {
	const options = {
		short: false,
		...partialOptions,
	};
...
export function toMultipleDigits(number: undefined, digits?: number): undefined;
export function toMultipleDigits(number: number | string, digits?: number): string;
export function toMultipleDigits(number: undefined | number | string, digits: number = 2): string | undefined {
...
export function formatNumber(number: number | string, partialOptions: Partial<FormatNumberOptions> = {}): string {
...
export function capitalizeText(text: string, partialOptions: Partial<CapitalizeTextOptions> = {}): string {
...
export function applyPlural(check: number): string {
	return check !== 1 ? "s" : "";
}

export function daySuffix(number: number): string {
